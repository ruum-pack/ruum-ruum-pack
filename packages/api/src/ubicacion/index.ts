...
export interface OpcionesUbicacion {
  /**
   * Solo shell nativo: en web devuelve `null` sin pedir permiso de
   * geolocalización (comportamiento legacy de app-usuario).
   */
  soloNativo?: boolean;
}
...
export function distanciaMetrosEntre(
  origen: Pick<Coordenadas, "lat" | "lng">,
  destino: Pick<Coordenadas, "lat" | "lng">
) {
...
export async function obtenerUbicacionActual(
  ...
  if (opciones.soloNativo && !esNativo()) return null;
    ...
    if (!esNativo()) {
      ...
      return await new Promise<ResultadoUbicacion>((resolve) => {
        navigator.geolocation.getCurrentPosition(
          ...
          (error) =>
            resolve(error.code === error.PERMISSION_DENIED ? { estado: "denegado" } : { estado: "no_disponible" }),
...
export async function observarUbicacionActual(
  alCambiar: (ubicacion: Coordenadas) => void
): Promise<CancelarObservacionUbicacion | null> {
  try {
    if (!esNativo()) {
