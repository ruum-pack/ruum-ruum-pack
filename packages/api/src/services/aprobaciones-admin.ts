...
export async function solicitarAprobacionAdmin(cliente: Cliente, entrada: {
  ...
  const rpc = cliente.rpc as unknown as (fn:string,args:Record<string,unknown>)=>Promise<{data:string|null;error:unknown}>;
  const { data, error } = await rpc("admin_solicitar_aprobacion", {
    p_tipo: entrada.tipo, p_capacidad: entrada.capacidad, p_recurso: entrada.recurso,
    p_recurso_id: entrada.recursoId ?? null, p_accion: entrada.accion, p_payload: entrada.payload ?? {}
  });
...
export async function listarAprobacionesPendientes(cliente: Cliente): Promise<SolicitudAprobacionAdmin[]> {
  ...
  const query = cliente.from as unknown as (tabla:string)=>any;
  const { data, error } = await query("solicitudes_aprobacion_admin").select("*").eq("estado","pendiente").order("creada_en",{ascending:false}).limit(100);
  if (error) throw normalizarError(error);
...
export async function decidirAprobacionAdmin(cliente: Cliente, id:string, aprobar:boolean, motivo:string, version:number) {
  await assertAdminPermission(cliente, "aprobaciones:aprobar");
  const rpc = cliente.rpc as unknown as (fn:string,args:Record<string,unknown>)=>Promise<{error:unknown}>;
  const { error } = await rpc("admin_decidir_aprobacion", {p_solicitud_id:id,p_aprobar:aprobar,p_motivo:motivo,p_version_esperada:version});
  if (error) throw normalizarError(error);
}
