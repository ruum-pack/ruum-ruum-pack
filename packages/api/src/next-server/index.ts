...
import { crearClienteServidor as crearClienteBase } from "../supabase";
 ...
 *
 * Solo para runtime Node de Next (server). No importar desde middleware
 * (edge) ni desde Client Components — ver `./next-browser`.
 */
export async function crearClienteServidorDesdeCookies() {
  ...
  return crearClienteBase(url, anonKey, {
    getAll() {
