-- RT-30 -- El conductor no debe quedarse varado en entrega_confirmada.
--
-- Desde entrega_confirmada el servidor decide el siguiente estado segun el
-- tipo de pago: anticipado con pago completado -> pago_completado; al cierre
-- -> pago_pendiente. El cierre operativo queda disponible desde pago_completado.
...
begin
  ...
  select estado, tipo_pago into v_estado_actual, v_tipo_pago
  ...
  case p_evento
    ...
    when 'resolver_pago_entrega' then
      if v_estado_actual <> 'entrega_confirmada' then
        raise exception 'Evento no permitido desde estado %', v_estado_actual;
      end if;
      if v_tipo_pago = 'anticipado' then
        if not exists (
          select 1
          from public.pagos pago
          where pago.traslado_id = p_traslado_id
            and pago.momento = 'anticipado'
            and pago.estado = 'completado'
        ) then
          raise exception 'No se puede cerrar la entrega: falta pago anticipado completado.';
        end if;
        v_estado_siguiente := 'pago_completado';
      else
        v_estado_siguiente := 'pago_pendiente';
      end if;
      v_evento_auditoria := 'registro_pago';

    when 'cerrar_servicio' then
      if v_estado_actual <> 'pago_completado' then
        raise exception 'Evento no permitido desde estado %', v_estado_actual;
      end if;
