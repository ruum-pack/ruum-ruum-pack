-- Repara un estado inconsistente detectado en producción: la migración
-- 20260711000119_piso_techo_precio_cotizado.sql quedó registrada como
-- aplicada en el historial de migraciones, pero su DDL nunca corrió contra
-- la base remota (excepto el ALTER TYPE, que además 20260711000120 vuelve a
-- intentar con IF NOT EXISTS). Esto dejaba a public.usuario_crea_traslado
-- con la firma vieja de 3 argumentos, mientras app-usuario ya llamaba la RPC
-- con 4 (agregando p_clave_idempotencia), produciendo un 404/PGRST202 en
-- /traslados/nuevo.
--
-- Todo lo de abajo es una repetición idempotente del contenido de 119 (no se
-- edita esa migración ya aplicada). Es seguro volver a correr esto en un
-- ambiente donde 119 sí se haya aplicado por completo: cada paso usa guardas
