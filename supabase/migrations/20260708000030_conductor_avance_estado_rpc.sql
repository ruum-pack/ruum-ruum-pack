-- Fase 6 — defensa en profundidad para traslados asignados.
-- El conductor ya no tiene UPDATE libre sobre su fila de traslado: no puede
-- modificar precio, ruta, origen/destino, contactos, conductor_id ni otros
-- campos operativos. Solo puede avanzar estados puntuales del camino feliz a
-- través de esta función security definer, que actualiza exclusivamente
-- `estado` y registra auditoría con actor = conductor.
