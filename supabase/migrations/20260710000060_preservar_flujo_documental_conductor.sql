-- RT-03 — Preserva el flujo documental anterior para conductores históricos,
-- además del flujo nuevo por solicitud.
...
begin
  ...
  if new.solicitud_id is not null then
    ...
    if v_estado='documentos_pendientes'
      ...
      and not exists (
        select 1 from (values ('licencia_frente'),('licencia_reverso'),('identificacion_oficial')) r(tipo)
        where not exists (select 1 from public.documentos_conductor d where d.conductor_id=new.conductor_id and d.tipo=r.tipo and d.estado='en_revision')
      ) then
