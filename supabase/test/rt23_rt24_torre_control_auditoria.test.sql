...
insert into auth.users(id,email,raw_app_meta_data,raw_user_meta_data,created_at,updated_at) values
  ('71111111-1111-4111-8111-111111111111','rt24-admin@local.test','{}','{}',now(),now()),
  ('74444444-4444-4444-8444-444444444441','rt24-rechazo@local.test','{}','{}',now(),now()),
  ('74444444-4444-4444-8444-444444444442','rt24-aprobacion@local.test','{}','{}',now(),now()),
  ('74444444-4444-4444-8444-444444444443','rt24-documento@local.test','{}','{}',now(),now());
...
select public.rechazar_solicitud_conductor_admin(
...
do $$ begin
  ...
  ) then raise exception 'RT-24: el rechazo final no quedó atribuido y trazado.'; end if;
end $$;
...
select public.aprobar_solicitud_conductor_admin(
...
do $$ begin
  ...
  ) then raise exception 'RT-24: la aprobación no materializó un conductor activo.'; end if;
  if not exists(
    select 1 from public.historial_estados_solicitud_conductor
  ...
  ) then raise exception 'RT-24: la aprobación no quedó atribuida y trazada.'; end if;
end $$;
...
select public.revisar_documento_conductor_admin(
...
declare v_id uuid;
begin
  if (select estado from public.solicitudes_conductor where id='73333333-3333-4333-8333-333333333333') <> 'requiere_correccion' then
    raise exception 'RT-24: el rechazo documental no solicitó corrección.';
  end if;
  if (select count(*) from public.historial_estados_solicitud_conductor
      where solicitud_id='73333333-3333-4333-8333-333333333333'
        and decision in ('rechazar_documento','solicitar_correccion')) <> 2 then
    raise exception 'RT-24: faltan eventos diferenciados para documento y expediente.';
  end if;

  select id into v_id from public.historial_estados_solicitud_conductor
  where solicitud_id='73333333-3333-4333-8333-333333333333' limit 1;
  begin
    update public.historial_estados_solicitud_conductor set motivo='Manipulado' where id=v_id;
    raise exception 'RT-24: el historial permitió una modificación.';
  exception when others then
    if sqlerrm not like '%historial de decisiones es inmutable%' then raise; end if;
  end;
end $$;
