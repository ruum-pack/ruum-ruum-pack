    ...
    <div className="mt-5 rounded-lg border border-ink/10 px-4 py-4">
      ...
      <div className="mt-4 grid gap-3">
        <label className="flex flex-col gap-1.5">
          <span className="font-body text-xs font-medium text-ink/70">Tipo de problema</span>
          <select
            value={tipo}
        ...
        <label className="flex flex-col gap-1.5">
          <span className="font-body text-xs font-medium text-ink/70">¿En qué momento ocurrió?</span>
          <select
            value={momento}
            onChange={(e) => setMomento(e.target.value as MomentoIncidencia)}
            className="rounded-lg border border-ink/50 bg-mist px-3 py-2 font-body text-sm"
            aria-label="Momento de la incidencia"
          >
            {MOMENTOS.map((opcion) => (
