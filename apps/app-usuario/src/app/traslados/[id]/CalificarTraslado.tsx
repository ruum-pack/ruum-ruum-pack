  ...
  return (
    <div className="mt-6 rounded-lg border border-signal/20 bg-signal-soft/35 px-4 py-4">
      ...
      <label className="flex flex-col gap-1">
        <span className="sr-only">Comentario de calificación</span>
        <textarea
          value={comentario}
          onChange={(evento) => setComentario(evento.target.value)}
          className="mt-3 min-h-24 w-full rounded-lg border border-ink/50 bg-mist px-3 py-2 font-body text-sm focus-visible:outline focus-visible:outline-[3px] focus-visible:outline-offset-1 focus-visible:outline-route-dark"
      ...
      <div className="mt-3 flex items-center gap-3">
        <Button onClick={enviar} disabled={pendiente}>
          {pendiente ? TEXTOS_CARGANDO.enviando : "Enviar calificación"}
        </Button>
