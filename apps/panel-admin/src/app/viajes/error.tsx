...
export default function ErrorViajes({ reset }: { reset: () => void }) {
  return (
    <div className="flex min-h-[50vh] flex-col items-center justify-center px-6 py-16 text-center">
      <h1 className="font-display text-xl font-semibold">No pudimos cargar los viajes</h1>
      <p className="mt-2 font-body text-sm text-ink/60">
        Falla temporal de conexión. Intenta de nuevo en unos segundos.
      </p>
      <div className="mt-6 flex flex-wrap justify-center gap-3">
        <button onClick={reset} className="inline-flex min-h-10 items-center justify-center rounded-lg border border-ink/20 bg-mist px-4 py-2 font-body text-sm font-medium text-ink transition hover:border-ink/40">
          Reintentar
        </button>
