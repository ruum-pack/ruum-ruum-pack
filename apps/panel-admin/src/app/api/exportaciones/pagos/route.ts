...
export async function GET(request:Request){
 ...
 try{
  const {data,error}=await cliente.from("pagos").select("id,traslado_id,monto,estado,registrado_en").order("registrado_en",{ascending:false}).limit(LIMITE_FILAS);
  if(error)throw error; const filas=data??[]; const csv=["id,traslado_id,monto,estado,registrado_en",...filas.map(f=>[f.id,f.traslado_id,f.monto,f.estado,f.registrado_en].map(celda).join(","))].join("\n");
