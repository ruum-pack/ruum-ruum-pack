...
export default function PaginaDashboard() {
  ...
  const [indicadores, setIndicadores] = useState<IndicadorAccionableDashboard[]>([]);
  ...
  useEffect(() => {
    let activo = true;
    const cargar = (esRefresco = false, manual = false) => cargarDashboard(esRefresco, manual, activo);
    void cargar();
    const intervalo = window.setInterval(() => void cargar(true), 30000);
    return () => {
      activo = false;
