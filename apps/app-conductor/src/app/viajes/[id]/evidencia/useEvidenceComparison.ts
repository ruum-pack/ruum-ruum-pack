...
export function useEvidenceComparison(trasladoId: string): EvidenciaComparada & { resultados: ResultadosComparacion | null } {
  ...
  const cargarEvidencias = useCallback(async () => {
    ...
    try {
      ...
      const [inicialRes, finalRes] = await Promise.all([
        cliente
      ...
      const evidenciaInicial = dataToInspeccion(inicialRes.data);
      const evidenciaFinal = dataToInspeccion(finalRes.data);
...
export function calcularComparacion(inicial: InspeccionEvidencia | null, final: InspeccionEvidencia | null): ResultadosComparacion {
