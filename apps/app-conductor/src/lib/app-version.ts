...
export async function fetchVersionPolicy(): Promise<VersionPolicy | null> { const client=crearClienteNavegador(); const {data,error}=await client.rpc("obtener_politica_version_app", { p_plataforma:"android", p_version_actual:process.env.NEXT_PUBLIC_APP_VERSION ?? "0.0.0" }); if(error) return null; return data as VersionPolicy; }
