...
export async function recordOperationalEvent(type: OperationalEvent, details: Record<string, unknown> = {}) {
  try { const client = crearClienteNavegador(); await client.rpc("registrar_evento_operativo_app", { p_tipo: type, p_version_app: "1.0.0", p_detalle: sanitize(details) as any }); } catch { /* observability must never break operation */ }
}
